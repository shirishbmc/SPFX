...
export default class HelloWorldWebPart extends BaseClientSideWebPart<IHelloWorldWebPartProps> {
  ...
  private handleButtonClick(): void {
    ...
    const image: HTMLImageElement = this.domElement.querySelector('#image');

    const apiUrl: string = ``; // Replace "/api/data" with your API endpoint
    const jsonData = [      // Define your JSON data here
    { name: 'Self service', guid: 'B0110j6JDxR' },
    { name: 'Item 2', guid: '789012' },
    { name: 'Item 3', guid: '345678' },    // Add more items as needed
  ];
  ...
  fetch(apiUrls, {
    ...
    .then(data => {
      ...
      const parts = shortenedLink.split('/');
      const spfxValue = parts[parts.length - 1]; // Extract the last part of the URL
      
      console.log('http://localhost:3000/get_image_link?link='+spfxValue); // Output: 3r2EvVl

      fetch('http://localhost:3000/get_image_link?link='+spfxValue, {
        method: 'GET',
      })
        .then(response => response.json())
        .then(data => {
       ...
       image.src='http://localhost:3000/images/output.png';
        })
